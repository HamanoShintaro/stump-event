...
import { ChevronLeft, Calendar, Compass, MapPin, Camera } from "lucide-react";
    ...
    (async () => {
      ...
      try {
        const { data: evs, error } = await supabase
          .from("stamp_events")
          .select("id, created_at, visitor_number, spot_id, route_id, memo, photo_url")
          .eq("user_id", user.id)
          ...
          >
            ...
            >
              ...
              <div style={{ position: "relative", zIndex: 2 }}>
                ...
                {/* 撮影された思い出写真 ＆ 感想メモ */}
                {(selectedEvent.photo_url || selectedEvent.memo) && (
                  <div style={{ background: "#FFFDF9", border: "1.5px dashed var(--accent-color, #C9A84C)", borderRadius: "10px", padding: "12px", margin: "0 0 20px 0", textAlign: "left", boxShadow: "0 2px 6px rgba(0,0,0,0.02)" }}>
                    <div style={{ fontSize: "0.72rem", color: "var(--accent-color)", fontWeight: "bold", marginBottom: "6px", display: "flex", alignItems: "center", gap: "4px" }}>
                      <span>📷</span> 旅の思い出
                    </div>
                    {selectedEvent.photo_url && (
                      <div style={{ width: "100%", maxHeight: "160px", borderRadius: "6px", overflow: "hidden", border: "1px solid #EBE5D9", marginBottom: selectedEvent.memo ? "8px" : "0" }}>
                        <img src={selectedEvent.photo_url} alt="思い出の写真" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>
                    )}
                    {selectedEvent.memo && (
